	...
	for _, resp := range resp_elems {
		...
		if ok {
			...
			if found {
				...
				status.OAuthSamlConfigEventBroadcast(vs_cache_obj.Name, vs_cache_obj.ServiceMetadataObj.CRDStatus, svc_mdata_obj.CRDStatus)
			...
			status.OAuthSamlConfigEventBroadcast(vs_cache_obj.Name, lib.CRDMetadata{}, svc_mdata_obj.CRDStatus)
	...
	if ok {
		...
		if found {
			...
			case lib.ChildVS:
				...
				status.OAuthSamlConfigEventBroadcast(vs_cache_obj.Name, vs_cache_obj.ServiceMetadataObj.CRDStatus, lib.CRDMetadata{})
			default:
