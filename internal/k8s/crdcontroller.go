	...
	if lib.AKOControlConfig().L4RuleEnabled() {
		healthMonitorEventHandler := cache.ResourceEventHandlerFuncs{
			AddFunc: func(obj interface{}) {
				...
				processed, _, err := lib.IsHealthMonitorProcessed(key, namespace, name, healthMonitorObj)
				if err != nil {
					utils.AviLog.Warnf("key: %s, msg: error: Error processing HealthMonitor. err: %s", key, err)
					return
				}
				if !processed {
					utils.AviLog.Warnf("key: %s, msg: HealthMonitor is not processed by ako-crd-operator", key)
					return
				}
			...
			UpdateFunc: func(oldObj, curObj interface{}) {
				...
				if oldHealthMonitorObj.GetResourceVersion() != curHealthMonitorObj.GetResourceVersion() {
					...
					// Validate HealthMonitor is processed by AKO CRD Operator
					processedOldObj, _, _ := lib.IsHealthMonitorProcessed(key, namespace, name, oldHealthMonitorObj)
					processedCurObj, _, _ := lib.IsHealthMonitorProcessed(key, namespace, name, curHealthMonitorObj)
			...
			DeleteFunc: func(obj interface{}) {
				...
				// Validate HealthMonitor was processed by AKO CRD Operator
				processed, _, err := lib.IsHealthMonitorProcessed(key, namespace, name, healthMonitorObj)
				if err != nil {
					utils.AviLog.Warnf("key: %s, msg: error: Error processing HealthMonitor. err: %s", key, err)
					return
				...
				healthMonitorNsName := namespace + "/" + name
...
// @AI-Generated // Generated by Cursor // Description: cleanupHealthMonitorToL4RuleMappings removes all HealthMonitor to L4Rule mappings for a deleted L4Rule
func (c *AviController) cleanupHealthMonitorToL4RuleMappings(key, l4RuleNsName string, l4Rule *akov1alpha2.L4Rule) {
	...
	// Extract HealthMonitor references from the deleted L4Rule's aviCrdRefs
	if l4Rule.Spec.BackendProperties != nil {
		for _, backendProperty := range l4Rule.Spec.BackendProperties {
			if backendProperty.AviCrdRefs != nil {
				for _, aviCrdRef := range backendProperty.AviCrdRefs {
					if aviCrdRef.Kind != nil && *aviCrdRef.Kind == "HealthMonitor" && aviCrdRef.Name != nil {
						...
						healthMonitorNsName := l4Rule.Namespace + "/" + *aviCrdRef.Name
						objects.SharedCRDLister().DeleteHealthMonitorToL4RuleMapping(healthMonitorNsName, l4RuleNsName)
...
// @AI-Generated // Generated by Cursor // Description: processL4RulesForHealthMonitor finds all L4Rules that reference the given HealthMonitor and re-queues them
