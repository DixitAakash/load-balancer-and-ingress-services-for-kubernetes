...
func (c *AviController) Start(stopCh <-chan struct{}) {
	...
	if utils.IsWCP() {
		...
		// HealthMonitor dynamic informer - runs when L4Rule is enabled AND AKO CRD Operator is enabled since they work together
		if lib.AKOControlConfig().L4RuleEnabled() && lib.IsAKOCRDOperatorEnabled() && c.dynamicInformers != nil && c.dynamicInformers.HealthMonitorInformer != nil {
