...
func (l *leader) ValidateL4RuleObj(key string, l4Rule *akov1alpha2.L4Rule) error {
	...
	for _, backendProperties := range l4RuleSpec.BackendProperties {
		...
		// Validate HealthMonitor CRDs referenced through aviCrdRefs
		for _, aviCrdRef := range backendProperties.AviCrdRefs {
			if aviCrdRef.Kind != nil && *aviCrdRef.Kind == "HealthMonitor" {
				if aviCrdRef.Name == nil {
					err := fmt.Errorf("HealthMonitor name is required in aviCrdRefs")
					rejectL4Rule(key, l4Rule, err)
					return err
				}

				// Validate HealthMonitor CRD exists and is processed
				hmKey := lib.HealthMonitor + "/" + l4Rule.Namespace + "/" + *aviCrdRef.Name
				processed, ready, err := lib.IsHealthMonitorProcessed(hmKey, l4Rule.Namespace, *aviCrdRef.Name)
				if err != nil {
					validationErr := fmt.Errorf("HealthMonitor validation failed for %s: %v", *aviCrdRef.Name, err)
					rejectL4Rule(key, l4Rule, validationErr)
					return validationErr
				}
				if !processed {
					validationErr := fmt.Errorf("HealthMonitor %s is not processed by ako-crd-operator", *aviCrdRef.Name)
					rejectL4Rule(key, l4Rule, validationErr)
					return validationErr
				}
				if !ready {
					validationErr := fmt.Errorf("HealthMonitor %s is not in ready state", *aviCrdRef.Name)
					rejectL4Rule(key, l4Rule, validationErr)
					return validationErr
				}

				// Validate group is ako.vmware.com for HealthMonitor
				if aviCrdRef.Group != nil && *aviCrdRef.Group != "ako.vmware.com" {
					validationErr := fmt.Errorf("HealthMonitor %s must have group 'ako.vmware.com', got '%s'", *aviCrdRef.Name, *aviCrdRef.Group)
					rejectL4Rule(key, l4Rule, validationErr)
					return validationErr
				}

				// Validate HealthMonitor type matches backend protocol
				if err := validateHealthMonitorTypeCompatibility(key, l4Rule.Namespace, *aviCrdRef.Name, backendProperties); err != nil {
					rejectL4Rule(key, l4Rule, err)
					return err
				}
			}
...
// @AI-Generated // Generated by Cursor // Description: validateHealthMonitorTypeCompatibility validates that HealthMonitor type matches backend protocol
func validateHealthMonitorTypeCompatibility(key, namespace, healthMonitorName string, backendProperties *akov1alpha2.BackendProperties) error {
	// Use dynamic client to get HealthMonitor object directly (similar to IsHealthMonitorProcessed)
	clientSet := lib.GetDynamicClientSet()
	if clientSet == nil {
		return fmt.Errorf("internal error in fetching HealthMonitor %s/%s object for type validation", namespace, healthMonitorName)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to get HealthMonitor %s for type validation: %v", healthMonitorName, err)
	}

	// Extract HealthMonitor type from spec
	spec, found, err := unstructured.NestedMap(object.UnstructuredContent(), "spec")
	if err != nil || !found {
		return fmt.Errorf("failed to get spec from HealthMonitor %s: %v", healthMonitorName, err)
	}

	hmType, found, err := unstructured.NestedString(spec, "type")
	if err != nil || !found {
		return fmt.Errorf("failed to get type from HealthMonitor %s spec: %v", healthMonitorName, err)
	}

	// Get backend protocol
	if backendProperties.Protocol == nil {
		return fmt.Errorf("backend protocol is not specified for HealthMonitor %s validation", healthMonitorName)
	}
	...
	case "TCP":
		if hmType != "HEALTH_MONITOR_TCP" {
			return fmt.Errorf("HealthMonitor %s has type '%s' which is not compatible with TCP protocol. Valid types: HEALTH_MONITOR_TCP", healthMonitorName, hmType)
		}
	case "UDP":
		if hmType != "HEALTH_MONITOR_UDP" {
			return fmt.Errorf("HealthMonitor %s has type '%s' which is not compatible with UDP protocol. Valid types: HEALTH_MONITOR_UDP", healthMonitorName, hmType)
		}
	case "SCTP":
		if hmType != "HEALTH_MONITOR_SCTP" {
			return fmt.Errorf("HealthMonitor %s has type '%s' which is not compatible with SCTP protocol. Valid types: HEALTH_MONITOR_SCTP", healthMonitorName, hmType)
		}
	default:
		return fmt.Errorf("unknown backend protocol '%s' for HealthMonitor %s validation", backendProtocol, healthMonitorName)
	}

	utils.AviLog.Debugf("key: %s, HealthMonitor %s type '%s' is compatible with backend protocol '%s'", key, healthMonitorName, hmType, backendProtocol)
	return nil
}
