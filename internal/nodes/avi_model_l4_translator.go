...
func buildPoolWithL4Rule(key string, pool *AviPoolNode, l4Rule *akov1alpha2.L4Rule) {
	...
	// Process aviCrdRefs for HealthMonitor CRDs
	buildPoolWithAviCrdRefs(key, pool, l4Rule.Spec.BackendProperties[index], l4Rule.Namespace, l4Rule.Name)
}

// @AI-Generated // Generated by Cursor // Description: buildPoolWithAviCrdRefs processes aviCrdRefs field from L4Rule to apply HealthMonitor CRDs to pools
func buildPoolWithAviCrdRefs(key string, poolNode *AviPoolNode, backendProperty *akov1alpha2.BackendProperties, namespace, l4RuleName string) {
	if backendProperty == nil || backendProperty.AviCrdRefs == nil || len(backendProperty.AviCrdRefs) == 0 {
		return
	}

	healthMonitorRefsSet := sets.NewString()
	for _, aviCrdRef := range backendProperty.AviCrdRefs {
		if aviCrdRef.Kind != nil && *aviCrdRef.Kind == "HealthMonitor" && aviCrdRef.Name != nil {
			// Validate group field (should be ako.vmware.com for HealthMonitor)
			if aviCrdRef.Group != nil && *aviCrdRef.Group != "ako.vmware.com" {
				utils.AviLog.Warnf("key: %s, msg: HealthMonitor %s/%s has unsupported group %s, expected ako.vmware.com", key, namespace, *aviCrdRef.Name, *aviCrdRef.Group)
				continue
			}

			// Get HealthMonitor CRD using dynamic informer
			obj, err := lib.GetDynamicInformers().HealthMonitorInformer.Lister().ByNamespace(namespace).Get(*aviCrdRef.Name)
			if err != nil {
				utils.AviLog.Warnf("key: %s, msg: error: HealthMonitor %s/%s not found. err: %s", key, namespace, *aviCrdRef.Name, err)
				continue
			}

			// Validate HealthMonitor is processed by AKO CRD Operator
			unstructuredObj := obj.(*unstructured.Unstructured)
			hmKey := lib.HealthMonitor + "/" + namespace + "/" + *aviCrdRef.Name
			processed, ready, err := lib.IsHealthMonitorProcessed(hmKey, namespace, *aviCrdRef.Name, unstructuredObj)
			if err != nil {
				utils.AviLog.Warnf("key: %s, msg: error: Error processing HealthMonitor %s/%s. err: %s", key, namespace, *aviCrdRef.Name, err)
				continue
			}
			if !processed {
				utils.AviLog.Warnf("key: %s, msg: HealthMonitor %s/%s is not processed by ako-crd-operator", key, namespace, *aviCrdRef.Name)
				continue
			}
			if !ready {
				utils.AviLog.Warnf("key: %s, msg: HealthMonitor %s/%s is not in ready state", key, namespace, *aviCrdRef.Name)
				continue
			}

			// Extract UUID from status
			status, found, err := unstructured.NestedMap(unstructuredObj.UnstructuredContent(), "status")
			if err != nil || !found {
				utils.AviLog.Warnf("key: %s, msg: error: HealthMonitor %s/%s status not found. err: %s", key, namespace, *aviCrdRef.Name, err)
				continue
			}

			uuid, ok := status["uuid"]
			if !ok {
				utils.AviLog.Warnf("key: %s, msg: error: HealthMonitor %s/%s uuid not found", key, namespace, *aviCrdRef.Name)
				continue
			}

			healthMonitorUUID, ok := uuid.(string)
			if !ok || healthMonitorUUID == "" {
				utils.AviLog.Warnf("key: %s, msg: error: HealthMonitor %s/%s has invalid uuid", key, namespace, *aviCrdRef.Name)
				continue
			}
			...
			// Update HealthMonitor to L4Rule mapping for cache management
			healthMonitorNsName := namespace + "/" + *aviCrdRef.Name
	...
	// Apply health monitor UUIDs to pool node only if healthMonitorRefs is not already specified
	if healthMonitorRefsSet.Len() > 0 {
		// Check if healthMonitorRefs is already specified in the backend properties
		if backendProperty.HealthMonitorRefs != nil && len(backendProperty.HealthMonitorRefs) > 0 {
			utils.AviLog.Debugf("key: %s, msg: healthMonitorRefs already specified, skipping aviCrdRefs for Pool %s", key, poolNode.Name)
			return
		}

		// Apply health monitor UUIDs from aviCrdRefs
		poolNode.HealthMonitorRefs = healthMonitorRefsSet.List()
		utils.AviLog.Debugf("key: %s, msg: Applied HealthMonitor CRDs from aviCrdRefs to Pool %s", key, poolNode.Name)
	}
}
