...
func (c *GatewayController) Start(stopCh <-chan struct{}) {
	...
	// Start ApplicationProfile informer only if AKO CRD Operator is enabled
	if lib.IsAKOCRDOperatorEnabled() && c.dynamicInformers.AppProfileCRDInformer != nil {
	...
	// Start HealthMonitor informer only if AKO CRD Operator is enabled
	if lib.IsAKOCRDOperatorEnabled() && c.dynamicInformers.HealthMonitorInformer != nil {
	...
	// Start RouteBackendExtension informer only if AKO CRD Operator is enabled
	if lib.IsAKOCRDOperatorEnabled() && c.dynamicInformers.RouteBackendExtensionCRDInformer != nil {
